...
graph LR
    subgraph Entry["Entry points"]
        CLI[auto_loop.py CLI flags]
        DRY[--dry-run mode]
    end

    subgraph Core["Core loop"]
        ORCH[Orchestrator]
        EMA[EMA Smoother b=0.7]
...
|---|---|---|
| `auto_loop.py` | root | CLI entry, loop orchestration, git operations |
...
| `CXASEvalsAdapter` | `adapters/cxas_evals.py` | Bridges evaluate.py to cxas-scrapi eval clients |
...
To prevent the LLM from repeatedly applying the same mutation type,
the planner tracks recent mutation types in `.auto-cxas/state/mutation_history.json`.
...
Mutation diversity guidance:
  Recent types (last 5): prompt_patch x2, cache_tune x1
...
```
new_baseline = 0.7 x current_baseline + 0.3 x new_score
```
...
flowchart LR
    ...
    F -->|unavailable| FB[FallbackPlanner rule-based]

    style FB fill:#6b7280,color:#fff
```

If the selected LLM is unavailable, the `FallbackPlanner` picks the next
untried mutation type for the weakest eval dimension without any LLM calls.
